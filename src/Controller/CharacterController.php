...
class CharacterController extends AbstractController
{
    ...
    /**
     * @Route("/{id}/fight", name="character_fight", methods={"GET","POST"})
     */
    public function fight(Request $request, Character $character): Response
    {   
        $a= rand( 0 , $character->getPower() );

        if ( ($this->getUser()->getPoweruser())  > $a )  {
            $this->addFlash(
                'success',
                'Vous avez gagné ! '.
                $character->getFirstName().' vous a seulement infligé '.$a.' dégats'
            );
        }
        else {
            $this->addFlash(
                'success',
                'Vous avez perdu ! '.
                $character->getFirstName().' vous a infligé '.$a.' dégats'
            );
        }
